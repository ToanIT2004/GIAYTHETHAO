   ...
   function get_address_user() {
      ...
      $.ajax({
         ...
         success: (res) => {
            ...
            res.forEach(user => {
               const isChecked = user.role == 1 ? 'checked' : '';
               const row = `
                  <div class="form-check">
                     <input style="cursor: pointer;" ${isChecked} value="${user.id}" class="form-check-input" type="radio" name="chose_address" id="chose_address"">
                     <div class="shadow  p-3 mb-3 bg-body-tertiary rounded">
                        <div class="d-flex justify-content-between">
                           <div>
                              <div class="d-flex">
                                 <span style="font-size: 13px;" class="fw-bold">${user.fullname}</span>
                                 <div class="border mx-2 ml-2"></div>
                                 <span class="fw-bolder" style="font-size: 13px;">(+84) ${user.number_phone}</span>
                              </div>
                              <span class="fw-bolder" style="font-size: 12px">${user.address}</span> <br>
                              <span class="fw-bolder" style="font-size: 13px">${user.wards},  ${user.district}, ${user.province}</span>
                           </div>
                           <div> 
                              <div class="text-secondary" data-user_address_id="${user.id}" id="delete_address_user" style="cursor: pointer;"><i class="bi bi-x-lg"></i></div>
                           </div>
                        </div>
                     </div>
                     <hr>
